...
bool RtpPcmaReceiver::supports_payload_type(uint8_t payload_type) const {
    return payload_type == kRtpPayloadTypePcma;
}

bool RtpPcmaReceiver::populate_packet(
    ...
    if (properties.codec != StreamCodec::PCMA && properties.codec != StreamCodec::UNKNOWN) {
        return false;
    }

    const int sample_rate = properties.sample_rate > 0 ? properties.sample_rate : kDefaultPcmaSampleRate;
    int channels = properties.channels > 0 ? properties.channels : kDefaultPcmaChannels;
    if (channels <= 0) {
        channels = kDefaultPcmaChannels;
    }
    ...
    for (size_t i = 0; i < sample_count; ++i) {
        decoded[i] = decode_alaw_sample(packet.payload[i]);
    }
