...
constexpr std::size_t kPerProcessPayloadBytes = 1152;
...
PerProcessScreamReceiver::PerProcessScreamReceiver(
    ...
    : NetworkAudioReceiver(config.listen_port,
      ...
      config_(config),
      expected_packet_size_(kProgramTagSize + kScreamHeaderSize + kPerProcessPayloadBytes) {
...
bool PerProcessScreamReceiver::populate_tagged_packet(
    ...
    if (packet.channels <= 0 || packet.channels > 64 ||
        ...
        log_warning("Parsed invalid audio format from PerProcess Scream packet for " + out_composite_source_tag +
                      ". SR=" + std::to_string(packet.sample_rate) +
    ...
    packet.audio_data.assign(audio_payload_start, audio_payload_start + kPerProcessPayloadBytes);
...
bool PerProcessScreamReceiver::process_and_validate_payload(
    ...
    if (!populate_tagged_packet(buffer, size, sender_ip_str, out_packet, out_source_tag)) {
